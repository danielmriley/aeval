  ...
  public:
    ...
    Expr mkImplCnd(Expr pre, Expr cand, bool out = true)
    {
      ...
      if (isOpX<FORALL>(cand))
      {
        ...
        im = simplifyBool(mk<IMPL>(pre, cand));
      if (printLog >= 2 && out)
                         ...
                         ExprSet &s, ExprVector &srcVars, ExprVector &dstVars, ExprSet &cands)
    {
      ...
      Expr newCand = cnd;
      if (!finalizeArrCand(newCand, nextMbp, invNum))
        return;
    ...
    void generatePhaseLemmas(int invNum, int cycleNum, Expr rel, Expr cnd)
    {
      ...
      for (int p = 0; p < mbpDt[invNum].paths.size(); p++)
      {
        ...
        for (int m = 0; m < path.size(); m++)
        {
          ...
          if (dAllMbp)
            cur_mbps.push_back({p, m}); // TODO: try other heuristics
      ...
      if (!dAllMbp && curPath >= 0)
        cur_mbps.push_back({curPath, curDepth});
                    ...
                    ExprVector &srcVars, ExprVector &dstVars, bool fwd)
    {
      ...
      Expr candImpl;
      if (isOpX<FORALL>(cnd))
        candImpl = mkImplCndArr(invNum, mbp, cnd);
      else
        candImpl = mkImplCnd(mk<AND>(ind, mbp), cnd);
      ...
      {
        if (isOpX<FORALL>(cnd) && !isOpX<FORALL>(c))
          continue;
        if (isOpX<FORALL>(c))
        {
          ExprVector cnds;
                      ...
                      set<cpp_int> &intCoefs)
    {
      ...
      for (auto &hr : ruleManager.chcs)
      {
        if (hr.dstRelation != invRel && hr.srcRelation != invRel)
          continue;
        ...
        for (auto &cand : sm.candidates)
          candsFromCode.insert(cand);
        for (auto &a : sm.intConsts)
          progConsts.insert(a);
        for (auto &a : sm.intCoefs)
          intCoefs.insert(a);
        ...
        if (lemma_found)
        {
          ...
          if (checkAllLemmas())
          {
            outs() << "Success after " << (i + 1) << " iterations "
                   << (rndStarted ? "(+ rnd)" : (i > defSz[invNum]) ? "(+ rec)"
                                                                    : "")
                   << "\n";
            printSolution();
            return true;
                      ...
                      ExprVector &dstVars, ExprSet &cands)
    {
      ...
      if (hasArray)
      {
        ...
        if (prjcts.empty())
          prjcts.push_back(mk<TRUE>(m_efac));
      }
                                 ...
                                 ExprVector &dstVars, int f = 0)
    {
      ...
      {
        if (mbpDt[invNum].hasEdge(f, *mbp))
          continue;
        ExprSet constr;
        if (f == 0)
          constr = {*mbp, prefs[invNum]};
        else
          constr = {mbpDt[invNum].tree_cont[f], ssas[invNum],
                    replaceAll(mk<AND>(mkNeg(mbpDt[invNum].tree_cont[f]), *mbp), srcVars, dstVars)};
        if (u.isSat(constr))
        {
          if (f == 0)
            mbpDt[invNum].addEdge(f, *mbp);
          else if (mbpDt[invNum].getExprInPath(f, *mbp) < 0) // to avoid cycles
            mbpDt[invNum].addEdge(f, *mbp);
        }
      }
    ...
    void strengthenMbpDecisionTree(int invNum, ExprVector &srcVars, ExprVector &dstVars)
    {
      ...
      if (!dStrenMbp)
        return; // by default, strenghtening is disabled

      for (auto &s : mbpDt[invNum].tree_edgs)
      {
        if (s.second.size() == 0)
          continue; // leaves, nothing to strenghten
        ...
        if (s.first == 0)
          ...
          constr = {mbpDt[invNum].tree_cont[s.first], ssas[invNum],
      ...
      for (int i = 0; i < qvits[invNum].size(); i++)
      {
        ...
        if (toRepl || u.implies(existing->range, newra)) // new one is more general
        {
          if (newer->closed || !existing->closed)
          {
            ...
            if (printLog >= 2)
              outs() << "Re"; // continued in `createArrAcc`
            return true;
          }
          if (!newer->closed && existing->closed)
            return false;
    ...
    void initializeAux(ExprSet &cands, BndExpl &bnd, Expr dcl, int cycleNum, Expr pref)
    {
      ...
      if (qvits[invNum].size() > 0)
        return;
      if (!containsOp<ARRAY_TY>(ssas[invNum]))
        return;
                           ...
                           Expr phaseGuard = NULL, Expr invs = NULL, bool fwd = true)
    {
      ...
      {
        if (printLog >= 3)
          outs() << "\nDataLearner2 is disabled, using RndLearnerV3\n";
        RndLearnerV3::getDataCandidates(cands, srcRel, phaseGuard, invs, fwd);
        return;
      }
      if (printLog && phaseGuard == NULL)
        outs() << "\nDATA LEARNING (DataLearner2)\n=============\n";
      if (phaseGuard == NULL)
        assert(invs == NULL && srcRel == NULL);

      DataLearner2 dl(ruleManager, m_z3, printLog); // Use DataLearner2
      vector<map<Expr, ExprSet>> poly;              // Keep similar structure to store results per iteration

      if (phaseGuard == NULL)
      {
        // run at the beginning, compute data once per relation
        map<Expr, ExprVector> m;
        getArrRanges(m);           // Assuming this helper is still valid/needed
        map<Expr, ExprSet> constr; // Constraints accumulated per relation

        for (int i = 0; i < dat; i++) // Loop 'dat' times for iterative learning
        {
          poly.push_back(map<Expr, ExprSet>()); // Store results for this iteration
          for (auto &dcl : decls)               // Iterate through all relevant declarations
          {
            ...
            ExprSet tmp;
            dl.getDataCands(tmp, dcl); // Retrieve candidates generated by computeData

            // Optional: Simplify or filter candidates if needed (like old filterTrivCands)
            // filterTrivCands(tmp); // Assuming this helper is still relevant

            poly.back()[dcl] = tmp; // Store candidates for this relation

            // Update constraints for the next iteration for this relation
            // This might need adjustment based on how constraints should evolve
            constr[dcl].insert(conjoin(tmp, m_efac));
          }
          if (poly.back().empty())
            break; // Stop if no new candidates found in an iteration
        }
      }
      else // phaseGuard != NULL
      {
        // run at `generatePhaseLemmas` for a specific relation `srcRel`
        ExprSet constr; // Constraints for this phase
        for (int i = 0; i < dat; i++)
        {
          poly.push_back(map<Expr, ExprSet>()); // Store results for this iteration
          ...
          // Optional: Simplify or filter candidates
          // filterTrivCands(tmp);

          poly.back()[srcRel] = tmp; // Store candidates for srcRel

          // Update constraints for the next iteration of this phase
          constr.insert(conjoin(tmp, m_efac));
          if (tmp.empty())
            break; // Stop if no new candidates found
        }
      }

      // Process collected candidates (similar to before)
      for (auto &c : poly) // Iterate through results from each 'dat' loop
      {
        for (auto &p : c) // Iterate through relations (Expr) and their candidates (ExprSet)
        {
          int invNum = getVarIndex(p.first, decls);
          ExprSet mutated_cands = p.second; // Start with candidates from DL2

          if (mut > 0) // Apply mutations if enabled
          {
            ExprSet tmp_after_mutation;
            // Mutate candidates (p.second) and store results in tmp_after_mutation
            mutateHeuristicEq(p.second, tmp_after_mutation, p.first, (phaseGuard == NULL));
            mutated_cands = tmp_after_mutation; // Use mutated candidates
          }

          // Add final candidates (original or mutated)
          for (Expr aa : mutated_cands)
          {
            Expr a = normalizePositive(simplifyArithm(aa), m_efac);
            if (printLog >= 3)
              outs() << "  Data candidate: " << a << "\n";
            if (containsOp<ARRAY_TY>(a))
              arrCands[invNum].insert(a); // Add array candidates
            else
              addDataCand(invNum, a, cands[p.first]); // Add non-array candidates using the specific V4 method
          }
        }
      }
    }
