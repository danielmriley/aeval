...
#include "deep/DataLearner2.hpp" // Include DataLearner2
...
namespace ufo
{
  class RndLearnerV4 : public RndLearnerV3
  {
    ...
    RndLearnerV4 (ExprFactory &_e, EZ3 &_z3, CHCs& _r, unsigned _to, bool _freqs,
                  ...
                  bool _dR, bool _dG, bool _d2, bool _doReg, int _debug) :
                           ...
                           Expr phaseGuard = NULL, Expr invs = NULL, bool fwd = true)
    {
      ...
      if (phaseGuard == NULL)
      {
        ...
        for (int i = 0; i < dat; i++) // Loop 'dat' times for iterative learning
        {
          ...
          for (auto & dcl : decls) // Iterate through all relevant declarations
          {
            ...
            if (!dl.computeData(dcl, m, constr, doReg)) continue; // Pass dcl, ranges, constraints
           ...
           if (poly.back().empty()) break; // Stop if no new candidates found in an iteration
        }
      }
      else // phaseGuard != NULL
      {
        ...
        for (int i = 0; i < dat; i++)
        {
          ...
          dl.computeDataPhase(srcRel, phaseGuard, invs, fwd, constr, doReg);
          ...
          constr.insert(conjoin(tmp, m_efac));
           if (tmp.empty()) break; // Stop if no new candidates found
        }
      }
  ...
  inline void learnInvariants4(string smt, unsigned maxAttempts, unsigned to,
       ...
       bool dSee, bool ser, bool d2, bool doReg, int debug)
  {
    ExprFactory m_efac;
    EZ3 z3(m_efac);
    SMTUtils u(m_efac);
    ...
    RndLearnerV4 ds(m_efac, z3, ruleManager, to, freqs, aggp, mut, dat,
                    ...
                    dFwd, dRec, dGenerous, d2, doReg, debug);
