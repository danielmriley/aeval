...
namespace ufo
{
  ...
  private:
    ...
    RATIONAL rmod(RATIONAL x, RATIONAL y) { // Modulus for rationals
      RATIONAL res;
      res = x/y;
    ...
    RATIONAL gcd(RATIONAL x, RATIONAL y) { // GCD for rationals
    ...
    RATIONAL gcd(vector<RATIONAL> v) { // GCD for a vector of rationals
      RATIONAL res = 0;
      if(!v.empty()) {
        res = v[0];
  ...
  private:
    ...
    void candsFromBasis(Expr srcRel) {
      ...
      if(debug ==  -1) outs() << "CANDS FROM BASIS: " << cnt << "\n";
    }
