...
namespace ufo
{
  ...
  static bool (*weakeningPriorities[])(Expr, tribool) =
      {
          [](Expr cand, tribool val)
          { return bool(!val); },
          [](Expr cand, tribool val)
          { return indeterminate(val) && containsOp<FORALL>(cand); },
          [](Expr cand, tribool val)
          { return indeterminate(val); }};
  ...
  public:
    RndLearnerV3(ExprFactory &efac, EZ3 &z3, CHCs &r, unsigned _to,
                 bool freqs, bool aggp, int _mu, int _da, int debug) : RndLearnerV2(efac, z3, r, _to, freqs, aggp, debug),
                               ...
                               ExprVector &varsRenameFrom, Expr rel, bool seed, bool fwd)
    {
      ...
      Expr newCand = eliminateQuantifiers(
          (fwd ? mk<AND>(candToProp, constraint) : mk<AND>(constraint, mkNeg(candToProp))),
      ...
      if (seed)
      {
        if (u.isTrue(newCand))
          ...
          return propagateRec(rel, newCand, true);
      }
      else
      {
    ...
    {
      if (printLog >= 3)
        outs() << "Adding candidate [" << invNum << "]: " << cnd << "\n";
      ...
      if (!isOpX<TRUE>(allLemmas) && u.implies(allLemmas, cnd))
        return false;
                                    ...
                                    bool regress = false)
    {
      ...
      for (int i = 0; i < cand->arity() - 1; i++)
      {
        ...
        {
          if (str->grows && regress)
            cnjs.insert(mk<GEQ>(qv, itRepl));
          else if (str->grows && !regress)
            cnjs.insert(mk<LT>(qv, itRepl));
          else if (!str->grows && regress)
            cnjs.insert(mk<LEQ>(qv, itRepl));
          else if (!str->grows && !regress)
            cnjs.insert(mk<GT>(qv, itRepl));
        }
        else
        {
    ...
    bool propagateRec(Expr rel, Expr cand, bool seed)
    {
      ...
      for (auto &hr : ruleManager.chcs)
      {
        ...
        {
          if (hr.isInductive && ruleManager.hasArrays[rel])
            continue; // temporary workarond
          propped.insert(&hr);
    ...
    void generalizeArrInvars(int invNum, SamplFactory &sf)
    {
      if (sf.learnedExprs.size() > 1)
      {
        ...
        for (auto &a : sf.learnedExprs)
        {
          ...
          for (auto &fhVar : sf.lf.getVars())
            postVars.erase(fhVar);
          for (auto &tmpVar : tmpVars)
            postVars.erase(tmpVar);
          if (postVars.size() > 0)
          {
    ...
    void assignPrioritiesForLearned()
    {
      for (auto &cand : candidates)
      {
        if (cand.second.size() > 0)
        {
          ...
          {
            b = simplifyArithm(b);
            if (!statsInitialized || containsOp<ARRAY_TY>(b) || containsOp<BOOL_TY>(b) || findNonlin(b))
    ...
    bool multiHoudini(vector<HornRuleExt *> worklist, bool recur = true)
    {
      ...
      for (auto &hr : worklist)
      {
        ...
        if (b || indeterminate(b))
        {
          ...
          outs() << "    CHC check succeeded\n";
      }
      if (!recur)
        return false;
      if (res1)
        return anyProgress(worklist);
      else
    ...
    void getQVcands(int invNum, Expr replCand, ExprSet &cands)
    {
      ...
      for (auto &q : qvits[invNum])
      {
        if (!contains(conjoin(se, m_efac), q->iter))
          continue;
        found = true;
    ...
    void getArrRanges(map<Expr, ExprVector> &m)
    {
      for (auto &dcl : decls)
        for (auto q : qvits[getVarIndex(dcl, decls)])
          if (q->closed)
            m[dcl].push_back(q->grows ? mk<MINUS>(q->postcond, q->precond) : mk<MINUS>(q->precond, q->postcond));
    }
    ...
    bool checkAllLemmas()
    {
      ...
      for (int i = ruleManager.wtoCHCs.size() - 1; i >= 0; i--)
      {
        ...
        if (b)
        {
          ...
          {
            if (printLog)
              outs() << "WARNING: Something went wrong" << (ruleManager.hasArrays[hr.srcRelation] || ruleManager.hasArrays[hr.dstRelation] ? " (possibly, due to quantifiers)" : "") << ". Restarting...\n";

            for (int i = 0; i < decls.size(); i++)
            {
              SamplFactory &sf = sfs[i].back();
              for (auto &l : sf.learnedExprs)
                candidates[i].push_back(l);
    ...
    bool createArrAcc(Expr rel, int invNum, Expr a, bool grows, vector<int> &extMbp, Expr extPref, Expr pre)
    {
      ...
      {
        if (printLog >= 3)
          outs() << "Preprocessing of counter " << a << " of relation " << rel << " fails  🔥\n";
        return false;
      }
      ...
      Expr fla = (ar->grows) ? mk<GEQ>(ar->qv, ar->precond) : mk<LEQ>(ar->qv, ar->precond);
      ExprSet tmp;
      getConj(pre, tmp);
      for (auto it = tmp.begin(); it != tmp.end();)
      {
      ...
      ar->range = simplifyArithm(normalizeDisj(mk<AND>(fla,
                                                       replaceAll(conjoin(tmp, m_efac), ar->iter, ar->qv)),
                                               invAndIterVarsAll));
    ...
    virtual void initializeAux(ExprSet &cands, BndExpl &bnd, Expr dcl, int cycleNum, Expr pref)
    {
      ...
      for (int i = 0; i < dstVars.size(); i++)
      {
        Expr a = srcVars[i];
        Expr b = dstVars[i];
        if (!bind::isIntConst(a) /*|| !contains(postconds[invNum], a)*/)
          continue;
