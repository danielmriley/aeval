...
def main():
    ...
    print(f"Found {len(benchmarks)} bv16 benchmarks. Running tests (PBE Mode) with {timeout}s timeout...")
    ...
    for bench in benchmarks:
        # Correct flags: --sygus (PBE) and --sygus-run (execute CVC5)
        ...
        try:
            ...
            if "Synthesized functions:" in output:
                status = "SUCCESS"
                success_count += 1
            elif "Success after" in output and "Synthesized functions" not in output:
                # This should not happen if --sygus is active, but if it does, it means flag issue
                status = "SAFE(INV)" 
                fail_count += 1
            else:
                fail_count += 1
