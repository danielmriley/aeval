; SAFE zero-extend version of cex3: three variables x, y, z all increment by 1
; The transition only fires when ALL next states satisfy the property
; This means the property is ALWAYS satisfied (no CEX exists)

(set-logic HORN)

(declare-fun inv ((_ BitVec 32) (_ BitVec 32) (_ BitVec 32)) Bool)

; Initial state: x = 0, y = 0, z = 0
(assert 
  (forall ((x (_ BitVec 32)) (y (_ BitVec 32)) (z (_ BitVec 32)))
    (=> (and (= x #x00000000) (= y #x00000000) (= z #x00000000)) (inv x y z))
  )
)

; Transition: all increment by 1, ONLY when all next states satisfy property
(assert 
  (forall ((x (_ BitVec 32)) (y (_ BitVec 32)) (z (_ BitVec 32))
           (x_next (_ BitVec 32)) (y_next (_ BitVec 32)) (z_next (_ BitVec 32)))
    (=> (and (inv x y z)
             (bvult (bvadd ((_ zero_extend 32) x) #x0000000000000002) #x0000000100000000)
             (bvult (bvadd ((_ zero_extend 32) y) #x0000000000000002) #x0000000100000000)
             (bvult (bvadd ((_ zero_extend 32) z) #x0000000000000002) #x0000000100000000)
             (= x_next (bvadd x #x00000001))
             (= y_next (bvadd y #x00000001))
             (= z_next (bvadd z #x00000001)))
        (inv x_next y_next z_next))
  )
)

; Property: all three must satisfy zext(v)+1 < 2^32
(assert 
  (forall ((x (_ BitVec 32)) (y (_ BitVec 32)) (z (_ BitVec 32)))
    (=> (and (inv x y z) 
             (not (and (bvult (bvadd ((_ zero_extend 32) x) #x0000000000000001) #x0000000100000000)
                       (bvult (bvadd ((_ zero_extend 32) y) #x0000000000000001) #x0000000100000000)
                       (bvult (bvadd ((_ zero_extend 32) z) #x0000000000000001) #x0000000100000000))))
        false)
  )
)

(check-sat)
