...
; Equivalent to bv64_cex1.smt2 but with pure BV property (no bv2int)
;
; Original property: (not (< (+ 1 (bv2int x)) 2^64))
; With zero_extend: NOT(bvult(bvadd(zext(x), 1), 2^64))
;
; Trace: x=0,1,2,...,2^64-1 (2^64 states)
...
; Property: zero_extend(x) + 1 < 2^64 (violated when x = 2^64-1)
(assert 
